...
def test_logging_format(caplog):
    ...
    prices = []
    history = []
    # create 6 slots: 2 past, current, and 3 future
        ...
        prices.append(
            {
                ...
                "startDate": start.isoformat(timespec="milliseconds").replace("+00:00", "Z"),
                "endDate": end.isoformat(timespec="milliseconds").replace("+00:00", "Z"),
            }
        )

    planner = WhenToHeatPlanner(_settings())

    # Call plan() which internally calls log_plan()
    result = planner.plan(
        now=now,
    ...
    # Verify result is a bool
    assert isinstance(result, bool)

    # Verify logging was called with expected parameters
    text = caplog.text
    assert "max_gap=" in text
    assert "min_run=" in text
    assert "happy_price=" in text
    assert "|" in text  # now marker present
    
    # Verify each hour prints exactly 4 symbols
    text = caplog.text
    assert "schedule=" in text
    schedule_str = text.split("schedule=", 1)[1].splitlines()[0].strip()
    parts = schedule_str.split(" ")
    assert parts
    for part in parts:
        # Remove leading hour digits
        rest = part.lstrip("0123456789")
        # Count slots by counting ON (🔥), OFF (❄️) and placeholder (?)
        slot_count = rest.count("🔥") + rest.count("❄️") + rest.count("?")
        assert slot_count == 4
